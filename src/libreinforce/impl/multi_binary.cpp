...
auto TypedMultiBinarySpace::_sample(
   size_t nr_samples,
   const std::optional< xarray< value_type > >& mask_opt
) -> xarray< value_type >
{
   if(nr_samples == 0) {
   ...
   if(not mask_opt.has_value()) {
      return xarray< value_type >{xt::random::randint(samples_shape, 0, 2, rng())};
   }
   ...
   auto samples = xt::empty< value_type >(samples_shape);
